...
def direct_binary_process(data: bytes) :
    ...
    used_us = int.from_bytes(data[4: 8], byteorder='big')
    int_feat_offset = 4 + 4
    ...
    return used_us, fp_feat, int_feat, ts_feat, ts_feat_2, x_dpi_bytes, mask
...
async def predict_ue(req: UeRequest):
    ...
    global ALL_INFER_TIME, ALL_FEAT_TIME1, ALL_FEAT_TIME2, ALL_PREPROCESS_TIME
    ...
    t_features = t1 - t0
    t0 = time.perf_counter()
    used_us, dpi_x, dpi_xc, dpi_x_start_ts, dpi_x_end_ts, dpi_x_dpi_bytes, dpi_mask = direct_binary_process(features_binary)
    t1 = time.perf_counter()
    t_preprocess = t1 - t0
    # df_clean = features[0]
    ...
    t1 = time.perf_counter()
    t_inference = t1 - t0
    ALL_INFER_TIME.append(t_inference * 1000.0)
    ALL_FEAT_TIME1.append(used_us / 1000.0)
    ALL_FEAT_TIME2.append(t_features * 1000.0)
    ALL_PREPROCESS_TIME.append(t_preprocess * 1000.0)
    if len(ALL_INFER_TIME) >= 100 and len(ALL_INFER_TIME) % 100 == 0 :
        ...
        mean_feat_time1 = np.mean(ALL_FEAT_TIME1)
        std_feat_time1 = np.std(ALL_FEAT_TIME1)
        mean_feat_time2 = np.mean(ALL_FEAT_TIME2)
        std_feat_time2 = np.std(ALL_FEAT_TIME2)
        ...
        print(f'inference time: {mean_infer_time}±{std_infer_time}ms')
        print(f'feature time1: {mean_feat_time1}±{std_feat_time1}ms')
        print(f'feature time2: {mean_feat_time2}±{std_feat_time2}ms')
        print(f'preprocess time: {mean_preprocess_time}±{std_preprocess_time}ms')
...
def test() :
    ...
    with open('test.bin', 'rb') as f :
        data = f.read()
    t0 = time.perf_counter()
    _, dpi_x2, dpi_xc2, dpi_x_start_ts2, dpi_x_end_ts2, dpi_x_dpi_bytes2, dpi_mask2 = direct_binary_process(data)
    t1 = time.perf_counter()
